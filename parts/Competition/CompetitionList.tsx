    ...
    <div className="flex flex-col gap-6 max-w-screen-lg mx-auto mt-8">
      <div className={"flex flex-row items-center justify-between"}>
        ...
        <Link href={"/competition/request"} className="btn btn-primary">
          <FaPlus className="inline text-base" /> Request Competition
        </Link>
      </div>
      {session === null ?
        <div className={"flex flex-col gap-4 mt-4 justify-center items-center"}>
          <h1 className={"text-gray-200 font-bold text-center text-4xl "}>
            You have to <Link className={"hover:underline"} href={'/login'}>Login</Link> First to See the Competition
          </h1>
        </div> :
        <div className={"flex flex-row flex-wrap gap-4 mt-2 relative justify-center items-center"}>
